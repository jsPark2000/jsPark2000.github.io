...
- 여기서 알아두어야 하는 특징이 몇개 있다.
  ...
  - 입력값의 차원(dimension)이 score에 주는 영향을 줄이는 것
    - **w<sub>1</sub>=[1, 0, 0, 0]** 이고 **w<sub>2</sub> = [0.25, 0.25, 0.25, 0.25]** , **x = [1, 1, 1, 1]** 이라고 하자
    - **w<sup>t</sup><sub>1</sub>x = w<sup>t</sup><sub>2</sub>x = 1**이 되는데 반해
    ...
    - 또한, L2 규제는 가중치의 값이 고르게 분포한 것을 선호한다는 것을 알 수 있다.
