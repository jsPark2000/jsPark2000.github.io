...
- **Binary Softmax classifier**
  ...
  - 위와 같은 식을 **P(y<sub>i</sub> = 1 | x<sub>i</sub> ; w)**,<br>한 클래스에 대한 확률로 해석 해볼 수 있다.
  - 여기서는 **이항 분류(Binary)** 이기 때문에, 정답과 정답이 아닌 클래스 2개만 존재하여
  - 정답인 클래스의 확률이 **P(y<sub>i</sub> = 1 | x<sub>i</sub> ; w)**이라면,<br>정답이 아닌 클래스의 확률은 **P(y<sub>i</sub> = 0 | x<sub>i</sub> ; w) = 1 - P(y<sub>i</sub> = 1 | x<sub>i</sub> ; w)**이다.
