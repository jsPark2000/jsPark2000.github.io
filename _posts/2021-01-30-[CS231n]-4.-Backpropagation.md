...
- 위의 과정을 회로도를 통해 간단하게 나타낼 수 있다.
  ...
  - backward pass는 오차역전파를 수행하는데, 출력에서 시작해서 연쇄법칙을 통해 입력에 대한 출력의gradient 값을 계산한다(빨간색).
...
- 위의 설명을 그림으로 정리해보자
  ...
  - **연쇄 법칙(chain rule)에 따라 각 게이트에서는 gradient를 가지고 게이트의 모든 입력이 되는 gradient를 구한다.** 
    - 이 부분은 Downstream gradient를 구하기 위해 Upstream gradient(dL/dz)에<br> local gradient인 (dz/dx)와 (dz/dy)를 곱하는 과정으로 이해할 수 있다.<br>다시 말하면 local input에 대한 경사를 구하는 것이다.
