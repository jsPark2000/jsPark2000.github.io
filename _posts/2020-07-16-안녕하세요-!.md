...
- 각 픽셀 값의 차이를 제곱한 다음 더한 것에 루트를 씌워주면 된다.

  ![zzzzzz](/assets/images/cs231n/zzzzzz.PNG)

- L1 distance 와 비교했을때 다른 결과가 나올 것 같지만<br>square root는 monotonic function(단조 함수) 이고, 픽셀값을 더하는 순서(order)가 유지되기 때문에<br>실제로는 NN에서 비슷한 결과를 낸다고 한다.
