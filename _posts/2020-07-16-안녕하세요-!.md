...
### 3-1. Vaildation sets for Hyperparameter tuning

---

- 앞에서 두 이미지의 거리를 계산하는 L1, L2 distance가 있었고, K-NN에서는 K가 있었다.<br>이것들의 공통점은 무엇일까?
...
- **Vaildation Set**
  ...
  - **하지만,  Test set을 기준으로 Hyperparameter를 설정하는 것은 위험하다.**<br>**test set에 overfit** 될 수 있기 때문이다.<br> (저는 test set에 잘 작동되는 hyperparameter를 설정한 것이기 때문에 실제 학습에는 test set을 사용하지 않지만 간접적으로 보면 overfit 될 수 있다는 의미로 해석하였습니다.)

    - Overfit: 학습된 데이터에만 좋은 성능을 보이고 학습된 데이터의 영역(분포)을 벗어나면 성능이 낮아지는 상황.

  - 그래서 이러한 문제를 해결하기 위해 등장한 것이 **Vaildation Set**이다.

  - 이렇게 하면 **vaildation set의 결과로 hyperparameter를 튜닝**하기 때문에 앞에서 발생하는 문제를 해결할 수 있다.

  - 강의에서는 vaildation set을 사용하는 이유에 대해 이렇게 표현했다.

    > you should think of the test set as a very precious resource.
    >
    > Evaluate on the test set only a single time, at the very end.

  - 정리하면, **Vaildation set은 Hyperparamter 값**을 찾기위한 가짜 test set 이고,<br>**Test set은 Hyperparameter 가 정해진 모델의 성능**을 평가하기 위한 것이라고 생각하면 된다.



#### 3-1-1. Cross-Vaildation
