...
-  **NN의 단점**
  ...
  3. L1, L2 distance를 사용하는데 있어서 문제점이 발생한다.<br>L1, L2의 경우 픽셀값의 차이만 가지고 두 이미지가 얼마나 비슷한지 판단하는데<br>이는 인지적, 의미적 거리를 고려하지 않기 때문에 아래와 같은 상황이 발생한다.<br>   ![samenorm](/assets/images/cs231n/samenorm.png)

     실제로 위의 사진들은 original 이미지를 기준으로 같은 L2 distance를 갖는다고 한다.

     

  4. 단점 3번의 또 다른 예시는 t-SNE 를 통해 확인 할 수 있다. L2 distance가 가까운 사진들을 붙여놓은 것인데, <br>  ![pixels_embed_cifar10](/assets/images/cs231n/pixels_embed_cifar10.jpg)

     자세히 보면 물체와는 상관없이 배경이 비슷하면 L2 distance가 가깝다고 판단하여 가까운 위치에 있는 모습을 볼 수 있다. 이는 고양이가 바다 배경에 있고, 배도 바다 배경에 있으면 L2 distance를 사용한 NN classifier가 둘을 같은 클래스로 분류할 가능성이 높다는 것을 의미한다.



## 4. 글을 마치며

이번 글에서는 **이미지 분류(Image Classification)**를 주제로,

Nearest Neighbor Classifier와 k - NN을 배우고, Hyperparameter tuning을 위한 Validation set과 k-fold에 대해서 알아 보았다.
